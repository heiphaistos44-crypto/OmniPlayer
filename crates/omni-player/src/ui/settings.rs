        ...
        .show(ctx, |ui| {
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    let drag = DragValue::new(&mut cfg.max_luminance)
                        .range(100.0..=10000.0)
                        .speed(50.0);
                ...
                ui.horizontal(|ui| {
                    ui.label("Port service sous-titres");
                    let drag = DragValue::new(&mut cfg.subtitle_service_port)
                        .range(1024..=65535);
                    if ui.add(drag).changed() { changed = true; }
                });
                ui.horizontal(|ui| {
                    ui.label("Port indexeur médias");
                    let drag = DragValue::new(&mut cfg.media_indexer_port)
                        .range(1024..=65535);
                    if ui.add(drag).changed() { changed = true; }
                });
                ...
                for (i, dir) in cfg.media_library.iter().enumerate() {
                    ui.horizontal(|ui| {
                        ui.label(RichText::new(dir).monospace().size(11.0));
                ...
                if ui.button("+ Ajouter un dossier").clicked() {
                    // Intégration rfd::FileDialog (à brancher)
                }
                ...
                ui.horizontal(|ui| {
                    ...
                    if changed {
                        ui.label(
                            RichText::new("● Modifications non sauvegardées")
